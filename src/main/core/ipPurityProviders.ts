...
import { purityNumber, purityObject, sanitizePurityDetails } from '../../shared/ipPurityScore'
...
function sameIp(actual: unknown, expected: string): boolean {
  try {
    return canonicalExitIp(actual) === canonicalExitIp(expected)
  } catch {
    return false
  }
}

export function parseAbuseIPDB(
  ip: string,
  raw: unknown
): IProxyPurityProviderAbuseIPDB | undefined {
...
function scamalyticsUrl(endpoint: string, key: string, ip: string): string {
  ...
  const url = new URL(
    endpoint.replaceAll('{ip}', encodeURIComponent(ip)).replaceAll('{key}', encodeURIComponent(key))
  )
...
export async function queryPuritySources(ip: string, config: IAppConfig): Promise<IpPurityDetails> {
  ...
  async function run<T>(
    ...
    if (!configured) {
      details.sourceStatus![source] = 'unconfigured'
      return
    }
    ...
    try {
      ...
      if (!parsed) {
        details.sourceStatus![source] = 'invalid'
        return
      }
      assign(parsed)
      ...
      else if (['ECONNABORTED', 'ETIMEDOUT', 'ERR_CANCELED'].includes(String(exception.code)))
        state = 'timeout'
  ...
  await Promise.all([
    run(
      ...
      true,
      () =>
        axios.get(`https://proxycheck.io/v3/${encodeURIComponent(ip)}`, {
          ...requestOptions,
          params: { key: config.ipPurityProxycheckApiKey || undefined }
        }),
      (raw) => parseProxycheck(ip, raw),
      (value) => {
        details.proxycheck = value
      }
    ),
    run(
      'scamalytics',
      ...
      parseScamalytics,
      (value) => {
        details.scamalytics = value
      }
    ),
    run(
      'abuseipdb',
      !!config.ipPurityAbuseIPDBApiKey?.trim(),
      () =>
      ...
      (raw) => parseAbuseIPDB(ip, raw),
      (value) => {
        details.abuseipdb = value
      }
    ),
    run(
      'ipapi',
      !!config.ipPurityIpapiApiKey?.trim(),
      () =>
        axios.post(
          'https://api.ipapi.is/',
          { q: ip, key: config.ipPurityIpapiApiKey!.trim() },
          requestOptions
        ),
      (raw) => parseIpapi(ip, raw),
      (value) => {
        details.ipapi = value
      }
    )

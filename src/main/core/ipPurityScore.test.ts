...
import { explainPurityScore, ipapiWeightedRisk, sanitizePurityDetails } from '../../shared/ipPurityScore'

const abuse = (risk: number): IProxyPurityProviderAbuseIPDB => ({ abuseConfidenceScore: risk, maxAgeInDays: 90 })
...
describe('weighted risk v2', () => {
  ...
  it('normalizes only participating weights, including a real zero result', () => {
    const value = explainPurityScore({ abuseipdb: abuse(0), proxycheck: { riskScore: 100, proxy: true } })
  ...
  it('does not substitute missing sources or unknown metadata with zero risk', () => {
    ...
    expect(explainPurityScore({ ipapi: { organization: 'Example', isVpn: false } }).score).toBeUndefined()
  ...
  it('gives full weights to all sources and explains the exact total', () => {
    const result = explainPurityScore({ abuseipdb: abuse(10), scamalytics: { score: 20 }, proxycheck: { riskScore: 50, vpn: true }, ipapi: { isAbuser: false, isVpn: true } })
  ...
  it('cannot average away serious abuse reports with three low readings', () => {
    const result = explainPurityScore({ abuseipdb: abuse(95), scamalytics: { score: 0 }, proxycheck: { riskScore: 0 }, ipapi: { isAbuser: false } })
  ...
  it('marks blacklists and compromised hosts without blaming VPN alone', () => {
    ...
    expect(explainPurityScore({ abuseipdb: abuse(0), proxycheck: { riskScore: 20, compromised: true } }).score).toBe(10)
  ...
  it('does not drop a VPN score discontinuously above its baseline', () => {
    const values = [49, 50, 51, 70, 100].map((riskScore) => explainPurityScore({ proxycheck: { riskScore, vpn: true } }).score!)
    expect(values).toEqual([...values].sort((a, b) => b - a))
  })

  it('sanitizes saved source records and never preserves credentials or raw errors', () => {
    const value = sanitizePurityDetails({ abuseipdb: { ...abuse(0), key: 'SECRET', reports: ['raw'] }, ipapi: { isAbuser: false, companyAbuserScore: 1, key: 'SECRET' }, sourceStatus: { abuseipdb: 'ok', ipapi: 'ok', scamalytics: 'SECRET' } })

...
const KeyInput: React.FC<{ name: ProviderKey; saved: string }> = ({ name, saved }) => {
  ...
  const save = (text: string): void => {
    ...
    void patchAppConfig({ [name]: text.trim() }).catch(() =>
      toast.error(t('proxies.purity.details.saveFailed'))
    )
  }
  return (
    <Input
  ...
  return (
    <>
      <SettingItem title="AbuseIPDB API Key" divider>
        {appConfig && (
          <KeyInput
            name="ipPurityAbuseIPDBApiKey"
            saved={appConfig.ipPurityAbuseIPDBApiKey ?? ''}
          />
        )}
      </SettingItem>
      <SettingItem title="ipapi.is API Key" divider>
        {appConfig && (
          <KeyInput name="ipPurityIpapiApiKey" saved={appConfig.ipPurityIpapiApiKey ?? ''} />
        )}
      </SettingItem>
    </>
  )
